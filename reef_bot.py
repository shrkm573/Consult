...
SYNTHESIS_TEMPLATE = """คุณเป็นผู้เชี่ยวชาญตู้ทะเลที่วิเคราะห์จากสองมุมมอง
...
จากข้อมูลของ Gemini ข้างต้น จงให้คำตอบที่ถูกต้องที่สุดสำหรับตู้นี้
- ถ้า Gemini ถูก ให้ยืนยันและเพิ่มเติมในจุดที่สำคัญ
- ถ้า Gemini ผิดหรือไม่ครบ ให้แก้ไขและให้คำตอบที่ถูกต้อง
ตอบตรงๆ ครบถ้วน ไม่ต้องอ้างถึง Gemini หรือ Claude — แค่ให้คำตอบที่ดีที่สุดเลย"""
...
def synthesize(question: str, gemini_ans: str) -> str:
    """Claude วิเคราะห์จาก Gemini แล้วให้คำตอบที่ดีที่สุด"""
...
def get_best_answer(question: str, user_id: str) -> str:
    """Gemini ตอบ → Claude กรองและให้คำตอบที่ดีที่สุด"""
    history = get_history(user_id)

    # Step 1: Gemini ตอบก่อน
    gemini_ans = ask_gemini(question, history)

    # Step 2: Claude วิเคราะห์และให้คำตอบสุดท้าย
    answer = synthesize(question, gemini_ans)
